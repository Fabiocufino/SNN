...
void draw_scatter_plot(const char* filename = "clusters_ntuple.root") {
    ...
    for (Long64_t entry = 0; entry < numEntries; ++entry) {
        IT->GetEntry(entry);
        Double_t x = cluster_R * TMath::Cos(cluster_phi);
        Double_t y = cluster_R * TMath::Sin(cluster_phi);
    ...
    for (Long64_t entry = 0; entry < numEntries; ++entry) {
        OT->GetEntry(entry);
        Double_t x = cluster_R_OT * TMath::Cos(cluster_phi_OT);
        Double_t y = cluster_R_OT * TMath::Sin(cluster_phi_OT);
    ...
    canvas->cd();
    redGraph -> GetXaxis()->SetRangeUser(-1300, 1300);
	redGraph -> GetYaxis()->SetRangeUser(-1300, 1300);
	
	blueGraph -> GetXaxis()->SetRangeUser(-1300, 1300);
	blueGraph -> GetXaxis()->SetRangeUser(-1300, 1300);
    ...
    redGraph->GetXaxis()->SetTitle("x");
    redGraph->GetYaxis()->SetTitle("y");
