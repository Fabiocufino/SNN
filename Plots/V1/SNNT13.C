...
static double TrackingLayers_distances[N_TrackingLayers] = {2.9, 1.95, 1.95, 2.05, 2.05, 2.55, 2.55, 3.05}; //[] = cm TrackingLayers_distances[i] = distance between i and i-1. In the first entry we could save the radial position of the first layer
...
void Define_tracker () {
    cout << "Definition of geometry" << endl;
    for (int is=0; is<N_strips; is++) {
        strip_y[is] = pitch*is;
    }
    cout << "Definition of x" << endl;
    //strip_x[0] = TrackingLayers_distances[0];
    strip_x[0] = 0;
    cout << "Entering the for" << endl;
    for (int itl=1; itl<N_TrackingLayers; itl++) {
        strip_x[itl] = strip_x[itl-1] + TrackingLayers_distances[itl];
    }
}
...
int Simulate_sighits (int pclass) {
    ...
    double x0    = strip_x[StartLayer]; // position vertex of track pairs on third layer if KoS=1
...
// Main routine
// ------------
void SNN_Tracking (int N_ev, int N_ep, int NL0, int NL1, int N_cl, double CF = 1., double Occ=0.00390625, int TrainingCode=7, bool ReadPars=false,
    ...
    for (int ic=0; ic<N_classes; ic++) {
        ...
        cout << "phivalues " << ic <<  " " << phivalues[ic] << endl;
        //phivalues[ic] = 0;
        //voglio solo eventi a carica positiva per compatibilità con v2
    ...
    S->SaveAs("S.pdf" , "pdf");
    C->SaveAs("C.pdf" , "pdf");
    E0->SaveAs("E0.pdf", "pdf");
    E1->SaveAs("E1.pdf", "pdf");
    BE->SaveAs( "BE.pdf", "pdf");
    SE->SaveAs( "SE.pdf", "pdf");
    W->SaveAs("W.pdf" , "pdf");
    Y->SaveAs("Y.pdf" , "pdf");
